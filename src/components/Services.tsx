    ...
    <ul className="grid gap-6 py-12 text-lg md:grid-cols-2 md:gap-x-12 lg:grid-cols-3">
      {allServices.map((service) => {
        return (
          <li
            key={service.title}
            className="relative h-[467px] rounded-lg bg-textLight-500 p-4"
          >
            <Image
              src={service.image}
              width={248}
            ...
            <hgroup className="absolute left-0 right-0 px-3 md:top-[58%]">
              <h2 className="py-4 text-center text-lg font-bold text-textDark-500">
                {service.title}
              </h2>
              <p className="px-3 text-base leading-7 text-darkDescription-500">
                {service.dscription}
              </p>
            </hgroup>
          </li>
